...
import com.laofeizhu.admin.modules.label.mapper.LabelUserTagMapper;
 ...
 */
@Service
public class LabelUserTagServiceImpl extends ServiceImpl<LabelUserTagMapper, LabelUserTag> implements ILabelUserTagService {

    @Override
    public void addTag(LabelUserTag tag) {
        //----------------------------------------------------------------------
        String pid = tag.getParentId();
        if(oConvertUtils.isNotEmpty(pid)) {
            //设置父节点不为叶子节点
            LabelUserTag parentTag = this.getById(pid);
    ...
    @Override
    public void editTag(LabelUserTag tag) {
        LabelUserTag p = this.getById(tag.getId());
        if(p==null) {
            ...
            int count = this.count(new QueryWrapper<LabelUserTag>().lambda().eq(LabelUserTag::getParentId, tag.getId()));
            if(count==0) {
                tag.setLeaf(true);
            }
            //----------------------------------------------------------------------
            ...
            if((oConvertUtils.isNotEmpty(pid) && !pid.equals(p.getParentId())) || oConvertUtils.isEmpty(pid)&&oConvertUtils.isNotEmpty(p.getParentId())) {
                ...
                int cc = this.count(new QueryWrapper<LabelUserTag>().lambda().eq(LabelUserTag::getParentId, p.getParentId()));
                if(cc==0) {
    ...
    public void deleteTag(String id) {
        LabelUserTag sysPermission = this.getById(id);
        if(sysPermission==null) {
            throw new JeecgBootException("未找到标签信息");
        }
        String pid = sysPermission.getParentId();
        int count = this.count(new QueryWrapper<LabelUserTag>().lambda().eq(LabelUserTag::getParentId, pid));
        if(count==1) {
    ...
    public void deleteByParentId(String parentId) {
        ...
        query.eq(LabelUserTag::getParentId, parentId);
